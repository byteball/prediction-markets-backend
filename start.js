...
async function discoverMarketAas() {
  ...
  Object.entries(factoryStateVars).forEach(([factory, stateVars]) => {
    Object.keys(stateVars).forEach((key) => {
      ...
      if ((factory === conf.factoryAas[0] && created_at <= conf.factoryUpgradeTimestamp[0]) || (factory === conf.factoryAas[1] && created_at <= conf.factoryUpgradeTimestamp[1]) || factory === conf.factoryAas[2]) {
